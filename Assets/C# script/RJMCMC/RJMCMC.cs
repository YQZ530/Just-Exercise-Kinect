...
void printEachJointLevelRotation()
{
    ...
    outputScript.printFloatArray(sumrot);
        ...
        for (int i = 0; i < 25; i++)
        {
            if (helperScript.ImportantJointRot[i]!= 0)
            {
                sumError += Mathf.Abs(helperScript.targetRot[i] - sumrot[i]);
            }
            
        }

        sumError /= (1f * helperScript.numImportantJointRot);
        outputScript.CreateSection("Avg Rotation joint Error");
        outputScript.printFloat(sumError);
    }
void  printSumCM()
    {
        float sumCM = 0f;

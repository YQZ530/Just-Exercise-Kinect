...
public class RJMCMCHelper : MonoBehaviour
{
   ...
   public IniFile CMDataFile;
    ...
    void LoadPresetAndTargetfiles()
    {
        ...
        //if(!DistDataFile.Load_File(Application.dataPath+ presetDataPath + "Qdistance.ini")) Debug.Log("Unable to load distance.ini"); 
        if(!RotDataFile.Load_File(Application.dataPath + presetDataPath + "Qrotation.ini")) Debug.Log("Unable to load rotation.ini"); 
        if(!CMDataFile.Load_File(Application.dataPath + presetDataPath + "Qcenter.ini")) Debug.Log("Unable to load cm.ini");
        //simpleTest();
        LoadTargetsFile();
    ...
    //public float CalculateMovementCosts(List<int> level)
    //{
    //    float result = 0f;
    //    result +=  CalculateDistCost(level)* distWeight; //* weigjt
    //   // CalculateRotCost() * rotWeight;
    //  //  CalculateCMtCost() * cmWeight;
    ...
    public float CalculateRotCost(List<int> level)
    {
        ...
        for (int k = 0; k < nRotJoint; k++)
        {
            ...
            else
            {
                ...
                float r2 = -1 * r * r / (2 * targetRot[k] * targetRot[k]);
                float r3 = (1 - Mathf.Exp(r2));
        ...
        //if(counterAffectJoint != 14) { print("counter joint has some problem "+ counterAffectJoint); }
        finalrot /= counterAffectJoint;

       // if(counterAffectJoint != 25) { Debug.LogError("joint is not 25!"); }
    ...
    void FindMaxDistTransition()
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            for (int j = 0; j < 10; j++)
            {
                ...
                for (int k = 0; k < nDistJoint; k++)
                {
                    sum += DistDataFile.Get_Float(k.ToString());
                    if(maxSum < sum) {
                        maxSum = sum;
